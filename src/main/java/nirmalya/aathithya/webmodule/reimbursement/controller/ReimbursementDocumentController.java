...
@Controller
public class ReimbursementDocumentController {

	Logger logger = LoggerFactory.getLogger(ReimbursementDocumentController.class);
	 ...
	 *
	 */
	@RequestMapping(value = "document/reimbursement/{docname}")
	@ResponseBody
	public ResponseEntity<byte[]> getDocument(@PathVariable(value = "docname") String docname) throws IOException {
		...
		File dir = ResourceUtils.getFile(env.getFileUploadReimbursementUrl());
		//File dir = ResourceUtils.getFile("G:/project/upload/reimbursement");
	...
	@RequestMapping(value = "document/reimbursement/thumb/{docname}")
	@ResponseBody
	public ResponseEntity<byte[]> getDocumentThumb(@PathVariable(value = "docname") String docname) throws IOException {
		...
		File dir = ResourceUtils.getFile(env.getFileUploadReimbursementUrl());
		//File dir = ResourceUtils.getFile("G:/project/upload/reimbursement/thumb");
		File file = new File(dir.getAbsolutePath() + "/" + docname);
		byte[] bytearr = Files.readAllBytes(file.toPath());
		if (docname.endsWith(".png")) {
			logger.info("Method : getDocument controller function starts");
