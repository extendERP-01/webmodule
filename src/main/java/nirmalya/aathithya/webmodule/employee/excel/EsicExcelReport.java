...
public class EsicExcelReport extends AbstractXlsView {

	Logger logger = LoggerFactory.getLogger(EpfExcelReport.class);

	double TESD_Empl_Epf = 0;
	double TESD_Empr_Epf = 0;
	...
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request,
		...
		try {
			...
			List<HrmsEsicExcelModel> accountLedger = (List<HrmsEsicExcelModel>) model.get("consumption");
			...
			cell1.setCellValue("Redimix Employee ESIC Details");
			...
			int i = 2;
			int j = 1;
			for (HrmsEsicExcelModel m : accountLedger) {
				...
				cell = row.createCell(4);
				cell.setCellValue(m.getEmpEsci());

				cell = row.createCell(5);
				cell.setCellValue(m.getEmpeerEsci());
			...
			for (HrmsEsicExcelModel m : accountLedger) {
				...
				TESD_Empl_Epf = TESD_Empl_Epf + m.getEmpEsci();
				TESD_Empr_Epf = TESD_Empr_Epf + m.getEmpeerEsci();

				cell = row.createCell(3);
				cell.setCellValue("Total:");

				cell = row.createCell(4);
				cell.setCellValue(TESD_Empl_Epf);

				cell = row.createCell(5);
				cell.setCellValue(TESD_Empr_Epf);
