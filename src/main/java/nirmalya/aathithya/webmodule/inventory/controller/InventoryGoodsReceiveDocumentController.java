...
@Controller
public class InventoryGoodsReceiveDocumentController {
	Logger logger = LoggerFactory.getLogger(InventoryGoodsReceiveDocumentController.class);
	...
	@RequestMapping(value = "document/invImg/{docname}")
	@ResponseBody
	public ResponseEntity<byte[]> getDocument(@PathVariable(value = "docname") String docname) throws IOException {
		...
		File dir = ResourceUtils.getFile(env.getFileUploadInventory());
	...
	@RequestMapping(value = "document/invImg/thumb/{docname}")
	@ResponseBody
	public ResponseEntity<byte[]> getDocumentThumb(@PathVariable(value = "docname") String docname) throws IOException {
		...
		File dir = ResourceUtils.getFile(env.getFileUploadInventory()+"/thumb");
		File file = new File(dir.getAbsolutePath() + "/" + docname);
		byte[] bytearr = Files.readAllBytes(file.toPath());
		if (docname.endsWith(".png")) {
			logger.info("Method : getDocument controller function starts");
