...
public class HrmsGradeRevisionAndIncrement {
	...
	public @ResponseBody JsonResponse<Object> saveGradeRevisionData(@RequestBody List<HrmsGraderevisionModel> testData,
		...
		if (!dataList.isEmpty()) {
			dataList.forEach(s -> s.setCalcTypeName(s.getCalculationType().contentEquals("1") ? "Flat Amount"
					: s.getCalculationType().contentEquals("2") ? "% Of Basic " : "% of Ctc"));

			double basic = 0;
			EmployeeSalaryIncementModel forbasic = dataList.stream()
					.filter(s -> s.getComponentId().contentEquals("EEM001")).findAny().get();
			if (forbasic.getCalculationType().contentEquals("1")) {
				basic = forbasic.getAmount();
			} else {
				basic = forbasic.getCtc() / forbasic.getAmount() * 100;
			}

			double ctc = forbasic.getCtc();
			for (EmployeeSalaryIncementModel a : dataList) {
				if (a.getCalculationType().contentEquals("1")) {
					a.setNewAnnualAmount((double) Math.round(a.getAmount()));
					a.setNewMonthlyAmount((double) Math.round(a.getAmount() / 12));
				} else if (a.getCalculationType().contentEquals("2")) {
					a.setNewAnnualAmount((double) Math.round(basic / a.getAmount() * 100));
					a.setNewMonthlyAmount((double) Math.round((basic / a.getAmount() * 100) / 12));
				} else {
					a.setNewAnnualAmount((double) Math.round(ctc / a.getAmount() * 100));
					a.setNewMonthlyAmount((double) Math.round((ctc / a.getAmount() * 100) / 12));
				}
	...
	@SuppressWarnings("unchecked")
	@RequestMapping(value = "view-grade-revision-promotion-save-increment", method = { RequestMethod.POST })
	public @ResponseBody JsonResponse<Object> saveIncrementData(@RequestBody List<EmployeeSalaryIncementModel> testData,
			Model model, HttpSession session) {
		logger.info("Method : saveIncrementData function starts");
		...
		try {
			res = restClient.postForObject(env.getPlanningUrl() + "saveIncrementData", testData, JsonResponse.class);
		} catch (RestClientException e) {
			e.printStackTrace();
		}
		String message = res.getMessage();
		...
		logger.info("Method : saveIncrementData function Ends");
		return res;
	}

	/*
	 * drop down for get job title by department
	 */
	@SuppressWarnings("unchecked")
	@GetMapping(value = { "/view-grade-revision-promotion-get-job-titles-by-dept-id" })
	public @ResponseBody JsonResponse<DropDownModel> getPropertyType(@RequestParam String departmentId) {
		logger.info("Method : get User Role List starts");

		JsonResponse<DropDownModel> res = new JsonResponse<DropDownModel>();

		try {
			res = restClient.getForObject(env.getEmployeeUrl() + "getJobTypeOnChange?deptId=" + departmentId,
					JsonResponse.class);
		} catch (Exception e) {
			e.printStackTrace();
		}
